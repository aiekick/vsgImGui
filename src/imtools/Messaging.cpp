...
void Messaging::Draw()
{
	...
	if (!puMessages.empty())
	{
		...
		{
			if (ImGui::MenuItem(ICON_FOAW_TRASH_O "##clear"))
			{
				Clear();
			}
		}
		...
		{
			if (ImGui::BeginMenu(ICON_FOAW_TRASH_O "##clear"))
			{
	...
	if (!puMessages.empty())
	{
		...
		{
			if (ImGui::MenuItem(ICON_FOAW_ANGLE_LEFT "##left"))
			{
				currentMsgIdx = ImMax(--currentMsgIdx, 0);
			}
            if (ImGui::MenuItem(ICON_FOAW_ANGLE_RIGHT "##right"))
			{
				currentMsgIdx = ImMax(++currentMsgIdx, (int32_t)puMessages.size() - 1);
			}
            if (ImGui::BeginMenu(ICON_FOAW_ANGLE_UP "##up"))
			{
